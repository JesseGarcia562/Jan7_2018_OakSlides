...
## Creating the 100 Bootstraps for q Lobata
```{r cars}
...
## Creating the 100 Bootstraps for q robur

```{r cars}
msmc='/u/home/j/jessegar/project-klohmuel/msmc-master/msmc'
bootstrapMSMC='/u/home/j/jessegar/project-klohmuel/msmc-tools/multihetsep_bootstrap.py'
qrobInput='/u/flashscratch/j/jessegar/forJesse/Qrob_PSMC/inputFiles'
output='$SCRATCH/Bootstraps/qrob_bootstraps'
qrobMultiHet<-list.files(qrobInput, 'postMultiHetSep.txt', full.names = T)
qrobMultiHet<-paste0(qrobMultiHet,collapse=' ')

command<-glue('python3 {bootstrapMSMC} -n 100 --seed 3 --nr_chromosomes 12 {output} {qrobMultiHet}')
...
```{r}
# myTerminal <- rstudioapi::terminalCreate()
# wd<-"/u/home/j/jessegar/project-klohmuel/oakTrees/code"
# settingUpTerminal<-glue("qrsh
#                         qrsh -l h_data=45G,h_rt=8:00:00
#                         cd {wd}
#                         module load R/3.5.0
#                         R
#         ")
# rstudioapi::terminalSend(id=myTerminal, text=settingUpTerminal) 



command<-paste0(read_lines("../code/TestingRStudioTerminalAPI.Rsh"), collapse = "\n")
rCmd<-parse_exprs(command)
cmds<-rCmd %>% map(~paste0(c(deparse(.x) , "\n") ,collapse=""))
cmds %>% map(~terminalSend(myTerminal,.x))


#"scp jessegar@dtn2.hoffman2.idre.ucla.edu:/u/home/j/jessegar/project-klohmuel/oakTrees/analysis/ /path/to/destination/for/copy"





dev.copy2pdf(file="../analysis/example.pdf")




quartz.save(file="../analysis/hi.png", type="png")
