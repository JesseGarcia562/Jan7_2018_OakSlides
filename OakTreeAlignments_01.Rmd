...
# Loading Q Robur Repeats and Runs of N File
...
rm(alignmentNotReversed)
rm(alignmentReversed)
rm(fixedQRoburAlignments)
...
snps <- snps %>% mutate(IsThisASNP= str_detect(QRoburAllele,"[ATCG]")  &  str_detect(QLobataAllele,"[ATCG]")   ) 

alignmentQRoburForwardRanges <- alignment %>% filter(QRoburStart < QRoburEnd)
alignmentQRoburBackwardRanges<- alignment %>% filter(QRoburStart > QRoburEnd)
alignmentQRoburBackwardRanges<- alignmentQRoburBackwardRanges %>% mutate(Temp=QRoburStart, QRoburStart=QRoburEnd) %>% mutate(QRoburEnd=Temp)
fixedQRoburAlignments<-bind_rows(alignmentQRoburForwardRanges, alignmentQRoburBackwardRanges)
rm(alignmentQRoburForwardRanges)
rm(alignmentQRoburBackwardRanges)


alignmentQRoburRanges<-GRanges(seqnames=fixedQRoburAlignments$QRobur,IRanges(start=fixedQRoburAlignments$QRoburStart, end=fixedQRoburAlignments$QRoburEnd),QLobataStart=fixedQRoburAlignments$QLobataStart, QLobataEnd=fixedQRoburAlignments$QLobataEnd )
...
## FInd all the alignments that aren't a repeat or N run

rangesThatWeCanKeep<-setdiff(alignmentQRoburRanges,qRoburRepeatsAndNGRange)


rangesThatWeCanKeepDf<-as_tibble(setdiff(alignmentQRoburRanges,qRoburRepeatsAndNGRange))



##### Annotate SNPs as repeat or not. We can Keep these snps. 

snpsQRoburRanges<-GRanges(seqnames=snps$QRoburNames,IRanges(start=snps$QRoburPosition, end=snps$QRoburPosition), QLobataAllele=snps$QLobataAllele, QRoburAllele=snps$QRoburAllele, IsThisASNP=snps$IsThisASNP)
#First find these snps
snpsWeCanKeep<-as_tibble(olRanges(query=rangesThatWeCanKeep, subject=snpsQRoburRanges)) %>%
  select(seqnames, Sstart) %>%
  mutate(QRoburRepeatOrN=FALSE, seqnames=as.character(seqnames))


rm(snpsQRoburRanges)
snps<-left_join(snps,snpsWeCanKeep, by=c("QRoburPosition" = "Sstart", "QRoburNames"="seqnames") )
rm(snpsWeCanKeep)


#If NA, then not in the setdiff ranges, then in a repeat or N run, then a repeat or n
snps<-snps %>% mutate(QRoburRepeatOrN=replace_na(QRoburRepeatOrN,TRUE))

snpsQRoburRanges<-GRanges(seqnames=snps$QRoburNames,IRanges(start=snps$QRoburPosition, end=snps$QRoburPosition), QLobataAllele=snps$QLobataAllele, QRoburAllele=snps$QRoburAllele, IsThisASNP=snps$IsThisASNP, QRoburRepeatOrN=snps$QRoburRepeatOrN)




overlaps<-as_tibble(olRanges(subject=alignmentQRoburRanges, query = snpsQRoburRanges))
rm(snpsQRoburRanges)

test=overlaps %>% 
  group_by(Sstart,Send,seqnames) %>%
  summarise(
    QRoburAlignmentStart=unique(Sstart),
    QRoburAlignmentEnd=unique(Send),
    RowOfAlignment=unique(Sindex),
    NumberOfSNPs=sum(IsThisASNP),
    NumberOfRepeatOrNSNPs=sum(IsThisASNP & QRoburRepeatOrN),
    QLobataDeletion=sum(str_detect(QLobataAllele, "\\.")),
    QRoburDeletion=sum(str_detect(QRoburAllele, "\\.")),
    QLobataHasThisManyMoreDeletions=QLobataDeletion-QRoburDeletion,
    QRoburHasThisManyMoreDeletions=QRoburDeletion-QLobataDeletion,
    QLobata2017Start=fixedQRoburAlignments[RowOfAlignment,]$QLobataStart,
    QLobata2017End=fixedQRoburAlignments[RowOfAlignment,]$QLobataEnd,
    AlignedLength=abs(QLobata2017End-QLobata2017Start) + QLobataHasThisManyMoreDeletions, 
    CheckingAlignedLength=abs(QRoburAlignmentEnd-QRoburAlignmentStart)
  ) %>%
  ungroup() %>%
  select(-Sstart, -Send)


check=test %>% filter(AlignedLength == CheckingAlignedLength) 

checkRanges<-GRanges(seqnames=check$seqnames,IRanges(start=check$QRoburAlignmentStart, end=check$QRoburAlignmentEnd) )


sum(check$NumberOfSNPs) - sum(check$NumberOfRepeatOrNSNPs)


sum(as_tibble(olRanges(rangesThatWeCanKeep, checkRanges))$width)
...
sum(width(setdiff(checkRanges, rangesThatWeCanKeep) ))





sum(check$NumberOfSNPs)
sum(check$AlignedLength)

mean(test$CheckingAlignedLength == test$AlignedLength)

hist(test$QRoburAlignmentEnd-test$QRoburAlignmentStart)
hist(test$QLobata2017End-test$QLobata2017Start)


summary(test$QLobata2017End - test$QLobata2017Start)

summary(test$CheckingAlignedLength - test$AlignedLength)

hi=test[test$CheckingAlignedLength - test$AlignedLength == 185,]

```



## Loading SNPS file  Fixing things
...
snpsRanges<-GRanges(seqnames=snps$QRoburNames,IRanges(start=snps$QRoburPosition, end=snps$QRoburPosition), QLobataAllele=snps$QLobataAllele, QRoburAllele=snps$QRoburAllele, IsThisASNP=snps$IsThisASNP)



overlaps<-as_tibble(olRanges(subject=alignmentQRoburRanges, query = snpsRanges))
rm(snpsQRoburRanges)
...
mean(test$AlignedLength == test$CheckingAlignedLength)
